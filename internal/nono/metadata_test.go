...
func TestMetadataStoreWritesCorrectJSON(t *testing.T) {
	...
	err := store.Write("nono", "/home/user/project")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	data, err := os.ReadFile(filepath.Join(tmpDir, "test-env", "nono", "metadata.json"))
	if err != nil {
...
func TestMetadataStoreCleanupRemovesMetadataDirectory(t *testing.T) {
	...
	metadataDir := filepath.Join(tmpDir, "test-env", "nono")
