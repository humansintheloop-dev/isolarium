...
- [x] **Task 6.3: `create` stops waiting once cloud-init has finished, and reports a degraded run**
  - TaskType: INFRA
  ...
  - Steps:
    - [x] Write the `internal/ec2/clone_test.go` cases first: exit 2 with `status: done` fails immediately, exit 1 fails immediately, exit 0 succeeds, and an unreachable instance is still retried until the budget expires
    - [x] Give `readinessLoop` the probe's exit code instead of the bool `succeeded` returns, since that bool collapses `not ready yet`, `finished degraded`, and `could not connect` into one answer; `session.exitCode` already exposes what is needed
    - [x] Classify the cloud-init probe's exits: 0 is ready, 1 and 2 are terminal, and the ssh transport failure that means sshd is not listening yet is the only case worth retrying
...
- [x] **Task 12.2: The full test suite and build pass with the EC2 backend present**
  ...
  - Steps:
    - [x] Run `go vet ./...` and fix any findings
...
- [x] **Task 13.2: A non-interactive `run --type ec2` command runs inside the tmux session, survives a dropped connection, and a re-run of the same command reattaches to it**
  - TaskType: OUTCOME
...
- [x] **Task 13.3: A non-interactive `run --type ec2` reports the command's own exit status, not tmux's, whether it started the session or reattached to it**
  ...
  - Steps:
    - [x] In `internal/ec2/tmux.go` add `StatusFilePath(session)` and a `WrapWithExitStatus(args, statusPath)` that shell-quotes the command with the existing `shellQuote` and appends the status write; test the quoting with arguments containing spaces and quotes
...
### 2026-08-20 14:31 - insert-task-after
cloud-init runs runcmd as root, so npm install -g @anthropic-ai/claude-code is the sudo global install the Claude Code documentation warns against: the binary lands in a root-owned npm tree that the ubuntu user cannot write, so its auto-update fails and Claude Code shows a notice at every start. The documented recommendation is the native installer, which puts the binary at ~/.local/bin/claude — already first on the PATH the instance publishes in /etc/environment.
...
### 2026-08-21 11:15 - mark-step-complete
...
### 2026-08-21 11:15 - mark-step-complete
CI lint job now compiles both tags. The step named go build, but every ec2 and e2e_ec2 file is a _test.go file that go build never reads, so the step also runs go vet with each tag - that is what actually type-checks the tagged tests and keeps them from rotting.

### 2026-08-21 11:15 - mark-step-complete
shellcheck test-scripts/*.sh exits 0 with no findings.

### 2026-08-21 11:22 - mark-step-complete
Final real-account runs on 2026-08-21 against account 500788673561 in us-west-1. test-ec2.sh: exit 0, ok internal/ec2 295.043s, one shared instance, cold start create-to-first-SSH 2m2s, cloud-init done 3m50s, user_data 3860 of 16384 bytes, no instance left running. test-ec2-e2e.sh: exit 0, TestClaudeInEC2_EndToEnd PASS in 242.37s with the agent commit message e2e-ec2-1787336506469 read back off the instance, ok cmd/isolarium 245.943s, no instance left running.
